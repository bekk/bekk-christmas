...
The codepen above takes screenshots, thanks to [html2canvas](https://html2canvas.hertzen.com/), and then I output the screenshot to a canvas. The canvas data is then processed to find the first occurring non-white pixel, and then the last occuring non-white pixel to get the first values that is needed. And to get the last piece of the puzzle, we do the same with the lower case character "x". 
