...
This retrives the individualt parts of the date into sperate variables.

### Fault tolerance

Elixirs, or rather Erlang VM/BEAMs proccess and supervisor model makes it easy to create fault tolerent, isolated applications that fit together and can handle the turbulent connectivity that might be present where you deploy your embeded system. Even just for internal error handeling, having the runtime there to kill a failing proccess and start it again with a known good state is better than the alternative in other languages with might in some instances be to completely crash.

### OTP releases

Nerves leverages OTP releases to create a deployable binary that is optimized and compiled for the target system. This is similar to (but not the same as) compiled languages like Go and Rust that produces a staticly linked binary.


## What is Nerves?

Nerves simplifies the prosses taking an Elixir application and packing it up into a firmware that can easily be deployed to almost any computer that can boot from a SD card or hard drive.
...
Even in small hobby projects Nerves can simplify deployment and maintenance. One of my projects is a dashboard on a Raspberry Pi touch screen, [nerves-pidash](https://github.com/ringvold/nerves-pidash). For the [first version](https://github.com/ringvold/pidash) of this the backend was written in Go. This worked well for a while. Go produces a staticly linked binary which makes deployments easy compared to other interpeted langugage where you need to make sure all dependencies is available. 

The hassel comes when the device loose power and I have to manually go and start the backend and open the browser to the correct url. This became tedious and of course I had the well known SD card corruption issue because of power loss.

Nerves' way for handling the filesystem and updates makes sure that power loss is not a issue and with the [nerves-web-kiosk/kiosk_system_rpi3](https://github.com/nerves-web-kiosk/kiosk_system_rpi3) the device starts a browser with a predefined url. Changes to the system is a breeze with OTA updates.
...
For my smart meter project the hard part was actually to find out how the binary protocol works. Reading the data from the device is easy with Circuits just a few lines of code. Admittedly this project is not in a workings state and some code has been lost. Remember to check in your code. That is something Nerves will not do for you. 😅 For the interested the project can be found at <https://github.com/ringvold/han_ams>.

But there are more serious projects out there. Some are basing there business on devices powered by Nerves!

### The more serious projects

The Nerves project web site has several show cases where you can read about how [Le Tote used Nerves to increasing warehose efficiency](https://www.nerves-project.org/customer-le-tote), how [Hop used Nerves to build the world's first self-serve beer kiosk powered by facial recognition technology](https://www.nerves-project.org/customer-hop) or how Nerves enables [open source smart farming through FarmBot](https://www.nerves-project.org/customer-farmbot).

One of my favorites is [Realtime Vehicle Tracking with Elixir and Phoenix by James Smith](https://www.youtube.com/watch?v=aLnSO6FQpHs) from ElixirConf 2017.

## Wrapping up

If you have a project involving a Raspberry Pi or another embedded device i recommend you to check out Nerves. It simplifies a lot of the hard parts about embedded programming and helps you focus on solving the problem at hand and not having to build you own deployment schemes.

For a good introduction by one of the co-authors of Nerves I recommend checking out Justin Schnecks talk [Functional IoT with Elixir and Nerves](https://www.youtube.com/watch?v=mrpQHZcy3CI) it goes a bit more into detail in some areas and have some awesome demos at the end!
