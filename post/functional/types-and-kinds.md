...
In the signature of `fmap` we see that the polymorphic type parameter `f` is simultaneously polymorphic in another type. The payoff is that we can supply a rich ecosystem of functionality that will allow code reuse for *any* functor (or other type class member) by supplying this class functionality while keeping the guarantees of strong, static typing.
