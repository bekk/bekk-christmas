...
Neural networks for text generation are usually designed to take a sequence of words (represented as vectors) as input, and output probabilities for the next word. The network is trained on sequences of words from a corpus of text documents we want it to mimic. We feed the network with sentences from the training corpus and the network try to predict the word that follows. After the network is trained we can generate text as follows:
