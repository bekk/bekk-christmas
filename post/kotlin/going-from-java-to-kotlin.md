...
However, there is a third option we tend to perfer, using a feature in Kotlin called String templates. This makes for very compact and highly readable statements like this:
...
We then refactored the converted class and instead made the function into an extension method on the `Response` class. To get rid of the rather ugly class parameter we also made it an `inline` function with a [reified](https://kotlin.christmas/2019/15 "When we need a concrete generic") type parameter. That ment we could get the class from the type parameter using `T::class.java`. 
...
## Final words
Ok, these were just a few simple examples, but hopefully something that gives you an idea of how to approach Kotlin from the Java world. It can be done gradually in small steps with the help of a handy plugin to speed things up. Thanksfully, it is still plenty of room for creative thinking and to discover and make use of all the features and capabilities in Kotlin. It should be fun!
