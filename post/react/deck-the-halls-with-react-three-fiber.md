...
ingress: Earlier this year I tried using [three.js](https://threejs.org/) within
  a React app. It was not straightforward, and I ran into quite a few problems.
  Since then, I've been eager to try out this library a co-worker told me about;
  react-three-fiber. Apparently it could solve all my problems (ok, maybe only a
  few) and make it super easy to create cool animations with few lines of code.
  Say no more!
links:
...
---
I am far from an expert in three.js, but I want to share my experience and give you a quick introduction to react-three-fiber. If you've caught yourself staring longingly at amazing animations, thinking "oh, I wish I could make that!", but not quite found the courage to start, then this might be something for you. Since it's component based, it's mainly plug-and-play! :partying_face:

## Getting Started

react-three-fiber is a React renderer for three.js, which in turn is built on the [WebGL API](https://www.khronos.org/webgl/). The [ninth article](https://javascript.christmas/2020/9) in this year's JavaScript calendar was about animation in three.js. If you haven't read it already, hop over and do so before you continue reading this article. We are going to recreate the example in the article, using react-three-fiber.

There are a couple of things to know before you get started. To display the animations, three.js renders a HTML `<canvas>` element with your content in a loop inside it. We need to prepare this canvas before we can "paint" on it (or animate, if you will). For this, three.js requires a _scene, camera and renderer_. In basic three.js, you would need to define and configure these yourself (as you probably saw in the ninth JavaScript article). Luckily, react-three-fiber does this for us, which means even less boilerplate code! We can use the Canvas-component directly:
...
We have a box, and we have an initial rotation (which is zero). Our hook doesn't do much yet. In his article on animations in three.js, Holger explained the details of the shaking animation he made. Let's borrow that nice code from Holger to shake it, baby!
...
If you want to compare the code to the versjon made using only `three.js`, you can find Holger's sandbox [here](https://codesandbox.io/s/cool-bartik-4v7xx?from-embed=&file=/src/index.js:2342-2359).
