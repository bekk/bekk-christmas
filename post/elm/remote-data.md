...
Modelling remote data with a custom type is a powerful technique, which can be adapted for different scenarios, depending on your app. You could for instance add an variant for `NotAsked`, if a request is only triggered by something else. And of you need the result of multiple requests to render something, you could combine the state for multiple requests in one custom type.
