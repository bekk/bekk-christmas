...
But callbacks can easily get messy. There is a reason why there is something called \_callback hell\_ :fire: (just Google it and see), and why this is not the preferred way of doing things. Let's move on to a new type of object that has another take on the matter.
...
In fact, this is also a way of using callbacks. We define what is to happen when we get a response, passing in a function to both \`then()\` and \`catch()\`. With Promises it's just being handled in a smoother way than our first example. But can it get \_even\_ better? Keep on reading!
