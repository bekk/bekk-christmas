...
The `noise()` function works similarly to `random()`, in that both functions return a number between 0 and 1. The random function gives you a different result every time, and is great for when you want uniform distribution. The upside of the noise function is that when you call the function with the same parameters two times, you get the same result. Similarly, calling the function two times with _similar_ parameters offers a _similar_ result. If you really want to get into some use cases of Perlin noise, then I highly recommend the introduction to Daniel Shiffman's [The Nature of Code](https://natureofcode.com/book/introduction/#intro_section6). He uses a language called Processing, which is what p5.js is based on. The syntax is pretty much Java and all the functions work in pretty much the same way as they do in p5, so the code should be easy to translate. Anyway, let's get back to our wind.
