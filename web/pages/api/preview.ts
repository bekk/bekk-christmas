...
const previewApiHandler: NextApiHandler = (req, res) => {
  if (!req || !req.query) {
    return res.status(500).json({ message: "Could not access request params. Internal error" });
  }
  ...
  if (type !== "post") {
    return res.status(400).json({ message: "Posts are the only thing that supports preview yet" });
  }
